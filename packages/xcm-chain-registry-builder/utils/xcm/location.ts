...
export function getGeneralIndex(multiLocationStr: string): number | null {
  try {
    ...
    return generalIndexObj?.generalIndex
      ? Number(generalIndexObj.generalIndex)
      : null;
  } catch (error) {
    ...
    try {
      ...
      if (reserveLocation.parents === '1') {
        if ('X1' in reserveLocation.interior) {
          if (
            Number(reserveLocation.interior.X1.Parachain) ===
            Number(targetParaId)
          ) {
            return ReserveType.Foreign;
          }
        ...
        if (
          'X2' in reserveLocation.interior &&
          Array.isArray(reserveLocation.interior.X2)
        ) {
          if (
            reserveLocation.interior.X2?.some((v) => {
              return (
                'Parachain' in v && Number(v.Parachain) === Number(targetParaId)
              );
            })
          ) {
            return ReserveType.Foreign;
        ...
        if (
          'X3' in reserveLocation.interior &&
          Array.isArray(reserveLocation.interior.X3)
        ) {
          if (
            reserveLocation.interior.X3?.some((v) => {
              return (
                'Parachain' in v && Number(v.Parachain) === Number(targetParaId)
              );
            })
        ...
        if (
          'X4' in reserveLocation.interior &&
          Array.isArray(reserveLocation.interior.X4)
        ) {
          if (
            reserveLocation.interior.X4?.some((v) => {
              return (
                'Parachain' in v && Number(v.Parachain) === Number(targetParaId)
              );
            })
